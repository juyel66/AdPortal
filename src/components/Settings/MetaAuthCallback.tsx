...
const MetaAuthCallback = () => {
  ...
  useEffect(() => {
    if (org_id) {
      ...
      setError(
        "No organization ID found. Please select an organization first.",
      );
      setLoading({ pages: false, accounts: false, saving: false });
    }
  }, [org_id]);
  ...
  const fetchAdAccounts = async (pageId: string) => {
    try {
  ...
  const handlePageSelect = async (page: Page) => {
    ...
    setSelectedAccount(null); // Reset selected account when page changes
    await fetchAdAccounts(page.id || page.page_id || "");
  };

  const handleSave = async () => {
    if (!selectedAccount || !selectedPage) {
      setError("Please select both a page and an ad account");
      return;
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Select your page first, then choose an ad account
          </p>
        </div>

        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          {/* Pages Section - Step 1 */}
            ...
            {!selectedPage ? (
              <div className="border border-gray-200 rounded-lg p-8 text-center bg-gray-50">
                <p className="text-sm text-gray-500">
                  Please select a page first to see ad accounts
                </p>
              </div>
            ) : loading.accounts ? (
              <LoadingSpinner />
              ...
              <div className="border border-gray-200 rounded-lg p-8 text-center">
                <p className="text-sm text-gray-400">
                  No ad accounts found for this page
                </p>
              </div>
            ) : (
              <div className="space-y-2">
        ...
        {/* Selection Summary */}
        {selectedPage && selectedAccount && !loading.saving && (
          <div className="mt-4 text-right text-xs text-gray-500 space-y-0.5">
            <div>Selected Page: {selectedPage.name}</div>
            <div>Selected Account: {selectedAccount.name || selectedAccount.account_id}</div>
